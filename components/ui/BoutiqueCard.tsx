...
export function BoutiqueCard({ icon, tag, title, desc, delay = 0 }: BoutiqueCardProps) {
  return (
    <motion.div
      ...
      className="group relative p-10 border border-border/70 bg-ivory-dark/35 rounded-[2rem] hover:bg-ivory transition-all duration-1000 ease-out flex flex-col gap-10"
    >
      ...
      <div className="absolute inset-x-0 bottom-0 h-1 bg-gold transform scale-x-0 group-hover:scale-x-100 transition-transform duration-1000 origin-left ease-[0.22,1,0.36,1] rounded-b-[2rem]"></div>
    </motion.div>
  );
}
